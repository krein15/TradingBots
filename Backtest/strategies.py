...
# ── Конфигурация, найденная перебором ─────────────────────────
# Отобрана ТОЛЬКО по периоду 2023-06..2025-06, проверена на
# 2025-06..2026-09, который в отборе не участвовал.
#
# Что она пережила:
#   смену режима рынка — обучение пришлось на рост (BTC +291%),
#     проверка на падение (BTC -25%, упали 17 пар из 19);
#   другую биржу и другой набор монет — на binance из 31 пары
#     совпадают с исходным набором лишь 7, результат тот же;
#   комиссию вдвое выше реальной (ноль достигается только при 0.4%
#     за сторону);
#   каждое полугодие периода по отдельности — все шесть в плюсе.
#
# Чего проверка НЕ снимает:
#   выживший отбор — список пар строится по сегодняшним объёмам,
#     монеты, ушедшие в ноль, в выборку не попали;
#   250 сделок на проверке при t-статистике ~1.3-1.8 — это
#     обнадёживает, но не доказывает;
#   шорты требуют перпетуальных фьючерсов, а там есть funding,
#     который здесь оценён, но не смоделирован посделочно.
FOUND_DONCHIAN_4H = {
    "timeframe":     "4h",
    "channel":       20,      # пробой максимума/минимума 20 свечей
    "atr_period":    14,
    "atr_mult":      2.5,     # стоп = 2.5 ATR, в среднем 6.5% от цены
    "rr":            3.0,     # тейк = 3 дистанции стопа
    "ema":           200,     # лонги только выше EMA200, шорты ниже
    "allow_short":   True,    # на проверке шорты дали больше лонгов
