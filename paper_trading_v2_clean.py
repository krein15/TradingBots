...
CONFIG = {
    ...
    "vol_avg_period":   20,
    "fibo_entry":       0.236,       # боковик: Фибо 23.6% — ждём откат
    "fibo_trend":       0.05,        # тренд: Фибо 5% — входим почти сразу на импульсе
    "max_stop_pct":     0.004,       # стоп >0.4% — не берём (0.4-0.5% WR=0%)
...
def read_market_regime():
    ...
    Возвращает: regime (str), confidence (int)
    SIDEWAYS   → fibo_entry (откат, классика)
    TREND_*    → fibo_trend (почти сразу на импульсе)
    """
    try:
        import json as _j
        ...
        # Проверяем свежесть — не старше 90 минут
        from datetime import datetime as _dt
        updated = _dt.fromisoformat(s.get("updated_at", "2000-01-01"))
        age_min = (_dt.now() - updated).total_seconds() / 60
        if age_min > 90:
            return "?", 0
        return regime, conf
    except Exception:
...
def find_signals(df, cfg, btc_trend="neutral", btc_chg=0.0, market_regime="?"):
    """
    Адаптивный вход по режиму рынка:
      SIDEWAYS / ?  → fibo_entry 23.6% — ждём откат к уровню
      TREND_UP/DOWN → fibo_trend  5%  — входим почти сразу на импульсе

    В тренде Фибо 23.6% не работает: импульс сильный,
    откат 23.6% слишком глубокий — цена уходит дальше пока ждём.
    При 5% входим ближе к цене пробоя — меньше риск не исполниться.
    """
...
def run_cycle(exchange, journal, cfg):
    ...
    if open_count < cfg["max_open_trades"]:
        ...
        # Читаем режим рынка — определяет стратегию входа
        market_regime, regime_conf = read_market_regime()
        is_trend   = market_regime in ("TREND_UP", "TREND_DOWN")
        fibo_used  = cfg.get("fibo_trend", 0.05) if is_trend else cfg["fibo_entry"]
        log(f"📊 Режим рынка: {market_regime} (уверенность {regime_conf}%)  "
            f"Фибо входа: {fibo_used:.1%}", cfg, show=True)
        ...
        for tf in cfg["timeframes"]:
            ...
            for sym in symbols[:80]:
                ...
                pending_sig = {
                    ...
                    "fibo_used":   fibo_used,   # какой Фибо использовался
                    "added_at":    now,
                    "bars_waited": 0,
                }
